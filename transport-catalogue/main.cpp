...
namespace {
    ...
    //-----------------------------------------------------Transport test -------------------------------------------------------------------------------------------------
    void TestFilling(TransportCatalogue& catalog) {
        ...
        Input::LoadTC(catalog, ifile);
        ifile.close();
        std::cout << "Filling test is successfully completed" << std::endl;
    }

    void TestStat(TransportCatalogue& catalog) {
        ...
        Stats::ExecuteQueries(catalog, ifile, std::cout);
...
void Transport_test() {
    ...
    TestFilling(catalog);
    TestStat(catalog);
    std::cout << "Catalogue test is complete" << std::endl;
}

void Load_test() {
    std::ifstream ifile("test3.txt");
    if (ifile.is_open()) {
         std::cout << "Success open file" << std::endl;
    }
    else {
    ...
    std::cout << "Load_test is complete" << std::endl;
}
...
#include "json_reader.h"
    ...
    Reader::LoadData(catalogue, doc.GetRoot());
    Reader::QueryProcessing(catalogue, doc.GetRoot(), ofile);
}
void Test() {
    JSON_test();
    Transport_test();
    Load_test();
    Reader_test();
    std::cout << "---------------------------------------- ALL TESTS IS OVER ----------------------------------------" << std::endl;
}

int main() {  
    json::Document doc = Load(std::cin);                                 //загрузка json документа из потока ввода
    TransportCatalogue catalogue;                                        //создание транспортного каталога
    Reader::LoadData(catalogue, doc.GetRoot());                          //заполнение транспортного каталога
    Reader::QueryProcessing(catalogue, doc.GetRoot(), std::cout);        //обработка запросов к транспортному каталогу и вывод в поток вывода